...
static std::vector<std::vector<double>> kinectPoints;
...
void coordtransfer(float x_pre, float y_pre, float & x_done, float & y_done)
{
	...
	x_standard1 = kinectPoints[0][0];
	y_standard1 = kinectPoints[0][1];
	x_standard2 = kinectPoints[1][0];
	y_standard2 = kinectPoints[1][1];
	x_standard3 = kinectPoints[2][0];
	y_standard3 = kinectPoints[2][1];
...
void coordtransfer(double x_kinect,double y_kinect,double z_kinect, float &x_screen, float &y_screen){
	double length = GetSystemMetrics(SM_CXSCREEN); //屏幕长
	double width = GetSystemMetrics(SM_CYSCREEN);  //屏幕宽
	double x_standard1 = kinectPoints[0][0];
	double y_standard1 = kinectPoints[0][1];//left bottom
	double z_standard1 = kinectPoints[0][2]/Div;
	double x_standard2 = kinectPoints[1][0];
	double y_standard2 = kinectPoints[1][1];//right bottom
	double z_standard2 = kinectPoints[1][2]/Div;
	double x_standard3 = kinectPoints[2][0];//left top
	double y_standard3 = kinectPoints[2][1];
	double z_standard3 = kinectPoints[2][2]/Div;

	y_kinect=y_kinect;
	double x_i,y_i;
	double kl0,kl1,kl2;
	double a[3][3]={x_standard1,x_standard2,x_standard3,y_standard1,y_standard2,y_standard3,z_standard1,z_standard2,z_standard3};
	double b[3][3];
	GetMatrixInverse(a,3,b);

	kl0=b[0][0]*x_kinect+b[0][1]*y_kinect+b[0][2]*z_kinect/Div;	
	kl1=b[1][0]*x_kinect+b[1][1]*y_kinect+b[1][2]*z_kinect/Div;
	kl2=b[2][0]*x_kinect+b[2][1]*y_kinect+b[2][2]*z_kinect/Div;

	double k=kl0+kl1+kl2;
	x_i=x_kinect/k;
	y_i=y_kinect/k;
	coordtransfer(x_i,y_i,x_screen,y_screen);
}
/***************************end blame****************************************/
					...
					if (SUCCEEDED(hr) && bTracked)
					{
						...
						//if(0)
						{
							...
							if (has_open_html && kinectPoints.size()<points_num && rightHandState == HandState_Closed && right_hand_transfer_flag == FALSE) {
								...
								temp.push_back(joints[JointType_ThumbRight].Position.X);
								temp.push_back(joints[JointType_ThumbRight].Position.Y);
								temp.push_back(joints[JointType_ThumbRight].Position.Z);
								kinectPoints.push_back(temp);
								fp << "===================thumb up!!====================\n";
								fp << "location{ x:" << kinectPoints.back()[0] << ",y:" << kinectPoints.back()[1] << ",z:"<< kinectPoints.back()[2]<<"};\n";
						...
						{
							/*******************BY Huyb***************/
							float temp_x_in_space = 0, temp_y_in_space = 0, temp_z_in_space = 0, temp_x_on_screen = 0, temp_y_on_screen = 0;
							float d_x = 0, d_y = 0;
							float x_ratio, y_ratio;
							...
							if (joints[JointType_HandRight].TrackingState == TrackingState_Tracked)
							{
								...
								temp_z_in_space = joints[JointType_HandRight].Position.Z;
								coordtransfer(temp_x_in_space, temp_y_in_space, temp_z_in_space, temp_x_on_screen, temp_y_on_screen);
								//fp<<temp_y_on_screen<<endl;
